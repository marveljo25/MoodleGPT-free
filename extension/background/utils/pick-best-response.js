    ...
    for (let i = 0; i <= str1WithoutSpaces.length; ++i) {
        ...
        for (let j = 1; j <= str2WithoutSpaces.length; ++j) {
            matrix[i][j] =
                i === 0
                    ? j
                    : Math.min(matrix[i - 1][j] + 1, matrix[i][j - 1] + 1, matrix[i - 1][j - 1] + (str1WithoutSpaces[i - 1] === str2WithoutSpaces[j - 1] ? 0 : 1));
        }
    }
 ...
 * @returns
 */
export function pickBestReponse(answer, arr) {
    let bestResponse = {
        element: null,
        similarity: 0,
        value: null
 ...
 * @returns
 */
export function pickResponsesWithSimilarityGreaterThan(answer, arr, score) {
    const responses = [];
    for (const obj of arr) {
        const similarity = sentenceSimilarity(obj.value, answer);
        if (similarity >= score)
 ...
 * @param similarity
 */
export function toPourcentage(similarity) {
