...
const testBtn = document.querySelector('#check-model');
if (!saveBtn)
...
if (!includeImagesCheckbox)
    throw new Error('Checkbox #includeImages not found');
// Save configuration
saveBtn.addEventListener('click', async () => {
    ...
    const includeImagesCheckbox = document.querySelector('#includeImages');
    if (!includeImagesCheckbox)
        throw new Error('Include Images checkbox not found');
    ...
    try {
        ...
        if (res.ok) {
            ...
            const modelInfo = data.data?.find((m) => normalizeModelId(m.id).includes(normalizeModelId(model)) ||
    ...
    const [logs, title, cursor, timeout, typing, mouseover, infinite, history, includeImages, useReasoning] = checkboxValues;
...
testBtn.addEventListener('click', async () => {
    chrome.storage.sync.get(['moodleGPT']).then(async (storage) => {
        ...
        try {
            const testResponse = await getResponse({ apiKey: cfg.apiKey, model: cfg.model, maxTokens: 50, useReasoning: cfg.useReasoning }, 'Hello, what is your model?');
