...
saveBtn.addEventListener('click', async () => {
    ...
    const [logs, title, cursor, typing, mouseover, infinite, timeout, history, includeImages] = checkboxValues;
...
testBtn.addEventListener('click', async () => {
    chrome.storage.sync.get(['moodleGPT']).then(async (storage) => {
        ...
        try {
            const testResponse = await getResponse({ apiKey: cfg.apiKey, model: cfg.model, maxTokens: 50 }, 'Hello, what is your model?');
